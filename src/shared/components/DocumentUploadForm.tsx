...
export function DocumentUploadForm({
  ...
  const handleDocumentChange = useCallback((updatedDocuments: Document[]) => {
    ...
    if (onDataChange) {
      const validFiles = updatedDocuments.filter(doc => (doc.file || doc.url) && !doc.needsReupload);
      const fileObjects = validFiles.filter(doc => doc.file).map(doc => doc.file!);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const validDocuments = documents.filter(doc => (doc.file || doc.url) && !doc.needsReupload);
      
      // Notify parent component with final data
      if (onDataChange) {
        const submissionData = {
          documents: documents,
          file: validDocuments.filter(doc => doc.file).map(doc => doc.file!),
