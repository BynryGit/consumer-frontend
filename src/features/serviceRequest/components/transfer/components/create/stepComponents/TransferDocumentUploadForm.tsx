...
interface TransferDocumentUploadFormProps {
...
const validateFile = (file: File): { isValid: boolean; error?: string } => {
  ...
  if (!ALLOWED_TYPES.includes(file.type)) {
    return { isValid: false, error: `File "${file.name}" is not a supported format. Only PDF, JPG, and PNG files are allowed` };
  }
...
const createDocumentCardsFromDocuments = (documents: Document[]): DocumentCard[] => {
...
const setupExistingDocumentSelections = (documents: Document[], documentTypeData: any) => {
...
export function TransferDocumentUploadForm({
  initialData = [],
  consumerId,
  onSave,
  onNext,
  ...
  onDataChange,
}: TransferDocumentUploadFormProps) {
  ...
  const [documentCards, setDocumentCards] = useState<DocumentCard[]>([
    { id: "doc_1", name: "Document", description: "Upload required documents", required: true }
  ]);
  const [selectedDocumentTypes, setSelectedDocumentTypes] = useState<{ [key: string]: string }>({});
  ...
  const removeDocumentCard = useCallback((cardId: string) => {
  ...
  const handleDocumentChange = useCallback((updatedDocuments: Document[]) => {
    ...
    if (onDataChange) {
      const validFiles = updatedDocuments.filter(doc => doc.file && !doc.needsReupload);
      const fileObjects = validFiles.map(doc => doc.file!);
      ...
      console.log('Document change - calling onDataChange with:', submissionData);
  ...
  const uploadFile = useCallback((
    ...
    const documentId = `${file.name}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  const handleDocumentTypeChange = useCallback((cardId: string, documentType: string) => {
    setSelectedDocumentTypes(prev => ({
      ...prev,
      [cardId]: documentType
    }));
  ...
  const isUploadDisabled = useCallback((cardId: string) => {
  ...
  const hasUploadedDocuments = useCallback(() => {
    return documents.some(doc => doc.file && !doc.needsReupload); // Check for valid files in memory
  }, [documents]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (onDataChange) {
        const submissionData = {
          documents: documents,
          file: validDocuments.map(doc => doc.file!),
        ...
        <div className="flex items-start">
          ...
          <div>
            <h4 className="font-medium text-blue-800 mb-1">
              {isEditMode ? "Update Document Requirements" : "Document Requirements"}
            </h4>
      ...
      {showSubmitButton && (
        <div className="flex justify-end">
          <Button
            ...
            aria-label={isLoading ? "Processing..." : (isEditMode ? "Update and Continue" : "Save and Continue")}
          >
            {isLoading 
